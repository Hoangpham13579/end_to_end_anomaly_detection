...
class I3Res50(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        nonlocal_mod = 2 if cfg.MODEL.NON_LOCAL else 1000
        self.layer1 = self._make_layer(Bottleneck, 64, cfg.MODEL.NUM_BLOCK_TEMP_KERNEL[0], stride=1, temp_conv=[1, 1, 1], temp_stride=[1, 1, 1])
        self.layer2 = self._make_layer(Bottleneck, 128, cfg.MODEL.NUM_BLOCK_TEMP_KERNEL[1], stride=2, temp_conv=[1, 0, 1, 0],
                                       temp_stride=[1, 1, 1, 1], nonlocal_mod=nonlocal_mod)
        self.layer3 = self._make_layer(Bottleneck, 256, cfg.MODEL.NUM_BLOCK_TEMP_KERNEL[2], stride=2, temp_conv=[1, 0, 1, 0, 1, 0],
                                       temp_stride=[1, 1, 1, 1, 1, 1], nonlocal_mod=nonlocal_mod)
        self.layer4 = self._make_layer(Bottleneck, 512, cfg.MODEL.NUM_BLOCK_TEMP_KERNEL[3], stride=2, temp_conv=[0, 1, 0], temp_stride=[1, 1, 1])
        self.avgpool = nn.AdaptiveAvgPool3d((1, 1, 1))
        self.fc = nn.Linear(512 * Bottleneck.expansion, 400)
        self.drop = nn.Dropout(cfg.MODEL.DROPOUT_RATE)
    ...
    def _make_layer(self, block, planes, blocks, stride, temp_conv, temp_stride, nonlocal_mod=1000):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion or temp_stride[0] != 1:
            downsample = nn.Sequential(nn.Conv3d(self.inplanes, planes * block.expansion, kernel_size=(1, 1, 1),
                                                 stride=(temp_stride[0], stride, stride), padding=(0, 0, 0),
    ...
    def forward(self, batch_data):
        # batch_inp shape: clips_num,c,t,h,w  # 24x3x16x224x224
        assert batch_data.dim() == 5, "Input data should be 5D tensor == single clip"
        feature = self.forward_single(batch_data)
        return feature
