...
from slowfast.utils_file import TaskInfo
# from advanTech.Datahub import Datahub, updateDeviceInfos
...
class VideoManager:
    ...
    def __init__(self, cfg):
        ...
        self.threshold = cfg.DEMO.ANOMALY_THRESHOLD
    ...
    def get_output_file(self, path, fps=24):
        """
    ...
    def display(self, task):
        ...
        """
        # Abnormal score of each clip
        abn_scores = task.clip_abn_score
        abn_scores = np.repeat(abn_scores, 16)

        for frame, abn_score in zip(task.frames[task.num_buffer_frames :], abn_scores):
            # if self.output_file is None:  # Case WEB_CAM
            if self.cfg.DEMO.WEBCAM > -1:  # Case WEB_CAM
                ...
                if abn_score > self.threshold:
                    frame = cv2.rectangle(frame, (0, 0), (self.display_width, self.display_height), (0, 0, 255), 3)
...
class ThreadVideoManager:
    ...
    def __init__(self, cfg):
        ...
        # self.cap = cv2.VideoCapture(self.source, cv2.CAP_V4L2) # For test stream video & upload to Azure storage
        self.cap = cv2.VideoCapture(self.source)  # For normal test
        ...
        if cfg.DEMO.OUTPUT_FPS == -1:
            ...
            self.output_fps = cfg.DEMO.OUTPUT_FPS
        if (cfg.DEMO.OUTPUT_FILE != "") and (cfg.DEMO.WEBCAM == -1):
            self.output_file = self.get_output_file(
                cfg.DEMO.OUTPUT_FILE, fps=self.output_fps
            )
    ...
    def start(self):
        ...
        # self.deviceInfo_thread = threading.Thread(
        #     target=updateDeviceInfos, args=[self.Datahub], name="DeviceInfo-Thread", daemon=True
        # )
        # self.deviceInfo_thread.start()
